...
A Japanese TTS using [Aratako/Irodori-TTS](https://github.com/Aratako/Irodori-TTS). The default is the approximately 766M-parameter `Aratako/Irodori-TTS-v4-Small`, which unifies text, reference-speech, and caption conditioning in one Rectified Flow DiT checkpoint. The released v3, v2, and v1 checkpoints remain selectable. Output is high-quality 48 kHz, but this OpenAI-compatible wrapper currently exposes text-only, no-reference inference and does not switch voices.
...
[shinshin86/sine-wave-tts](https://github.com/shinshin86/sine-wave-tts) turns Japanese text into deterministic beeping sine-wave tones for robots, mascots, and game characters. It follows Japanese readings and accents through Kuromoji and outputs 44.1 kHz mono 16-bit WAV. It runs entirely on CPU, downloads no model weights, and pins the upstream `v0.1.0` tag by default.

This engine is **electronic vocalization, not intelligible human speech**: listeners cannot recover the words from the audio alone. The OpenAI `voice` value combines one of five speakers (`default`, `chirpy`, `deep`, `robotic`, `songful`) with one of seven emotions (`neutral`, `joy`, `sad`, `angry`, `surprise`, `calm`, `fear`) as `speaker:emotion`, for example `chirpy:joy`. The same text and settings always produce the same sound.

The upstream Node.js server already implements `/v1/audio/speech`; this project adds a thin compatibility wrapper for the shared root and `/v1/voices` endpoints. WAV and raw 44.1 kHz mono 16-bit PCM are supported. License: MIT; there are no model weights.
...
The default language is `ja` (Japanese). When `--chatterbox-prompt-wav` is provided, the `clone` voice becomes available and uses the reference audio. A GPU runtime is recommended. In our Colab L4 validation, the Chatterbox process used 3,606 MiB after a synthesis request; actual usage depends on the hardware, software stack, and input. Every generated file retains Resemble AI's imperceptible Perth watermark. License: MIT for both the code and V3 weights.
...
[microsoft/VibeVoice-Realtime-0.5B](https://huggingface.co/microsoft/VibeVoice-Realtime-0.5B) is the lightweight single-speaker streaming variant of [microsoft/VibeVoice](https://github.com/microsoft/VibeVoice). Its [pinned upstream model card](https://huggingface.co/microsoft/VibeVoice-Realtime-0.5B/blob/6bce5f06044837fe6d2c5d7a71a84f0416bd57e4/README.md) reports roughly 300 ms to first audible speech on the native streaming path (hardware dependent). It accepts streaming text internally and supports robust long-form generation up to about 10 minutes. This OpenAI-compatible wrapper returns a completed WAV response rather than exposing the upstream WebSocket streaming protocol, so that first-audible figure is not a latency measurement of this wrapper.

The engine uses Microsoft's precomputed `.pt` prompt caches. `voice="default"` selects `VIBEVOICE_DEFAULT_SPEAKER`; any ID from `/v1/voices` can be passed directly. The default is the experimental Japanese preset `jp-Spk1_woman`. Custom reference audio and voice cloning are intentionally unavailable in this release, as are multi-speaker synthesis and speed control. English is the supported language; Japanese, German, French, Italian, Korean, Dutch, Polish, Portuguese and Spanish presets are experimental and may behave unpredictably. The bundled Qwen2.5-0.5B tokenizer dependency is pinned separately; its files are Apache 2.0. The wrapper also includes the restricted-unpickler workaround proposed in [upstream PR #397](https://github.com/microsoft/VibeVoice/pull/397) for [PyTorch compatibility issue #392](https://github.com/microsoft/VibeVoice/issues/392), retaining an allowlist instead of trusting arbitrary pickle globals.
...
[k2-fsa/OmniVoice](https://github.com/k2-fsa/OmniVoice) is a 0.6B diffusion-language-model-style TTS supporting more than 600 languages, including Japanese. It provides three modes: `auto` (no reference), `design` (speaker attributes such as gender, age, pitch, accent or whisper), and `clone` (a 3–10 second reference clip plus transcript). The installer pins source commit `38e992b` and model revision `c5fdb5c`, downloads the ungated checkpoint without `HF_TOKEN`, and creates a Python 3.12 environment with PyTorch 2.8.0/CUDA 12.8. GPU is recommended; the wrapper also has a CPU path, although generation will be much slower.
...
[Aratako/MioTTS-Inference](https://github.com/Aratako/MioTTS-Inference) is an LLM-based TTS. A standard LLM backbone (default [`Aratako/MioTTS-1.7B`](https://huggingface.co/Aratako/MioTTS-1.7B), a fine-tune of `Qwen/Qwen3-1.7B-Base`) autoregressively generates **MioCodec** tokens at 25 Hz, which the codec decodes to 44.1 kHz audio. The low frame rate (few tokens per second) is what makes synthesis fast.

**Architecture in this repo (three processes).** MioTTS' client talks to an OpenAI-compatible `/v1/chat/completions` server and relies on the chat template embedded in the model. Upstream documents `llama-server -hf …` or `vllm serve …` for that role, but **no prebuilt Linux CUDA `llama-server` binary is published**, so we host the GGUF (`Aratako/MioTTS-GGUF`, default `MioTTS-1.7B-Q8_0.gguf`) with a **prebuilt [`llama-cpp-python`](https://github.com/abetlen/llama-cpp-python) CUDA wheel** (`--miotts-llama-cuda`, default `cu124` — match Colab's CUDA, `cu121`–`cu125`). We then start `run_server.py` (the synthesis backend that loads MioCodec and decodes tokens) and proxy its `/v1/tts` as OpenAI-compatible `/v1/audio/speech`. The proxy passes the synthesis `timings` and `token_count` back as `x-miotts-timings` / `x-miotts-token-count` response headers for speed observability.

**Voices.** `default` uses the shipped preset selected by `--miotts-default-preset` (default `jp_female`). You can also pass a preset name directly as the `voice` (`jp_female`, `jp_male`, `en_female`, `en_male`). For your own speaker, set `--miotts-prompt-wav` and use `voice="clone"` (the reference audio is encoded on the fly). Sampling is exposed via `--miotts-temperature` / `--miotts-top-p` / `--miotts-repetition-penalty` / `--miotts-max-tokens` (defaults match upstream `DefaultLLMParams`).
...
A 0.9B LLM-based TTS by Vyvo. A `Qwen3-0.6B` backbone autoregressively emits **kyutai/mimi** audio tokens (32 codebooks), which the Mimi codec decodes to 24 kHz audio. There is no separate upstream package — inference is the plain `transformers` snippet from the model card, so the wrapper runs it in-process (Python 3.12 venv with `torch==2.9.1+cu128` / `transformers==5.0.0`). All checkpoints are ungated — no `HF_TOKEN` needed.
...
- [`Vyvo/Vyvo-Multilingual-JA-FT-v0.1`](https://huggingface.co/Vyvo/Vyvo-Multilingual-JA-FT-v0.1) — **Japanese-only** fine-tune of the multilingual base (a single Japanese speaker), weights **MIT**. Like the EN-FT, it is a derivative of the base, **not a newer-generation model**, and shares the exact same architecture, token layout, and Mimi codec. **Note: we were not able to verify normal-quality Japanese generation with this checkpoint in our own Colab testing — see the verification note below. We recommend the base checkpoint for Japanese.**

**Voices.** All checkpoints are fundamentally zero-shot cloning models with **no built-in speaker**, so every request needs a reference audio *and* its transcript. Set both `--vyvo-prompt-wav` and `--vyvo-prompt-text` and call with `voice="clone"` (`voice="default"` maps to the same path). Without them the wrapper returns a 4xx, mirroring the GPT-SoVITS contract — it does not silently fall back to a random voice. The transcript must match the clip (upstream notes that transcript-free / zero-shot cloning collapses into short, unrelated fragments). Sampling is exposed via `--vyvo-temperature` / `--vyvo-top-p` / `--vyvo-repetition-penalty` / `--vyvo-max-new-tokens` / `--vyvo-min-new-tokens` (defaults match the model card: 0.7 / 0.9 / 1.1 / 9600 / 960). For voice cloning, only use reference audio you have rights to (consent of the speaker).

Verified on Colab T4: the engine, the `/v1` endpoints, and the trycloudflare tunnel come up, and synthesis returns a valid 24 kHz WAV end-to-end through the public URL — English on the base / EN-FT checkpoints and Japanese on the **base** checkpoint. The base also handles cross-lingual cloning (a Japanese reference voice speaking English).

**We were not able to verify normal-quality Japanese output from the `Vyvo-Multilingual-JA-FT-v0.1` checkpoint in our own testing.** With the default sampling settings, the zero-shot clones we generated did not come out as usable speech — outputs tended to be very short, low-voiced or non-speech, and generation often ran to the token limit (very slow) instead of finishing cleanly. We observed the same with both an English male reference and a clean Japanese male reference (FLEURS, CC-BY-4.0), and lowering `--vyvo-temperature` to 0.3 did not change the result on our setup. We have not identified the cause, so this is a limitation of our own verification rather than a confirmed problem with the checkpoint. For now we recommend the base checkpoint for Japanese.

Note that autoregressive generation on a T4 is slow (~100 s for a one-sentence clip, and much slower when the JA-FT checkpoint collapses); an L4 / A100 is recommended for interactive use.

License: code is **Apache-2.0**; weights are **Apache-2.0** (`Vyvo-Multilingual-v0.1`) or **MIT** (`Vyvo-Multilingual-EN-FT-v0.1`, `Vyvo-Multilingual-JA-FT-v0.1`) — all allow commercial use. However, inference always loads the **kyutai/mimi** codec, which is **CC-BY-4.0** — commercial use is permitted but **attribution to Kyutai is required**. See the license table below.
...
| MioTTS (code) | MIT | — | OK | Repo: `Aratako/MioTTS-Inference`. LLM client (`/v1/chat/completions`) + MioCodec decode. Ungated weights, no `HF_TOKEN` |
| MioTTS-1.7B (default) / MioTTS-0.6B | — | Apache 2.0 | OK | Backbone fine-tunes of `Qwen3-1.7B-Base` / `Qwen3-0.6B-Base` (both Apache 2.0). Default shipped GGUF is `MioTTS-1.7B-Q8_0.gguf` |
| MioTTS-0.4B / 1.2B / 2.6B | — | LFM Open License v1.0 | OK under revenue cap | LFM-backbone variants. Commercial use allowed below the $10M annual-revenue cap (same family as LFM2.5-Audio-JP) |
...
| Vyvo-Multilingual-JA-FT-v0.1 | — | MIT | OK (license) | Japanese-only fine-tune of the base (single speaker). Derivative, not a newer generation; same architecture / token layout / Mimi codec. ⚠️ Could not verify normal-quality generation on our side; prefer the base for Japanese |
...
- Vyvo-Multilingual-JA-FT-v0.1 (Hugging Face)
  https://huggingface.co/Vyvo/Vyvo-Multilingual-JA-FT-v0.1
