...
| MioTTS | _Colab verification pending_ (GPU recommended, ~4-6GB VRAM, T4 expected to work; LLM-based — a Qwen3-1.7B-Base backbone hosted via a prebuilt llama-cpp-python CUDA wheel generates MioCodec tokens at 25 Hz → 44.1 kHz; ungated weights, no `HF_TOKEN`; preset voices or zero-shot cloning. **Default presets are non-commercial** — clone your own voice for commercial use) | Japanese / English |
