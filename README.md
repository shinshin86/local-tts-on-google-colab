...
| Vyvo-Multilingual | **Not yet verified on Colab** — code paths are in place but end-to-end Colab verification is pending (GPU recommended, ~2-4GB VRAM expected; 0.9B Qwen3-0.6B-backbone LLM-TTS that emits kyutai/mimi tokens → 24 kHz; runs in-process via plain transformers, no upstream repo; ungated weights, no `HF_TOKEN`). **Zero-shot cloning only — no built-in speaker**: every request needs `--vyvo-prompt-wav` + `--vyvo-prompt-text` (otherwise 4xx). Code + Vyvo weights Apache-2.0; the kyutai/mimi codec is CC-BY-4.0 (attribution) | English / Japanese |
...
#@markdown ---
#@markdown Vyvo-Multilingual (GPU recommended ~2-4GB VRAM, English/Japanese, voice cloning required)
...
#@markdown - **No HF_TOKEN needed**: weights ([Vyvo/Vyvo-Multilingual-v0.1](https://huggingface.co/Vyvo/Vyvo-Multilingual-v0.1)) are ungated.
#@markdown - Fundamentally a zero-shot cloning model with **no built-in speaker**: every request needs a reference audio + its transcript. Set both `VYVO_PROMPT_WAV` and `VYVO_PROMPT_TEXT` and call with `voice="clone"` (`default` maps to the same path). Without them the wrapper returns a 4xx — same contract as GPT-SoVITS.
#@markdown - License: code and Vyvo weights are **Apache-2.0** (commercial use OK). The **kyutai/mimi** codec is **CC-BY-4.0** — attribution required. For voice cloning, only use reference audio you have rights to (consent of the speaker).
VYVO_HF_MODEL = "Vyvo/Vyvo-Multilingual-v0.1"  #@param {type:"string"}
...
A 0.9B LLM-based multilingual TTS using [Vyvo/Vyvo-Multilingual-v0.1](https://huggingface.co/Vyvo/Vyvo-Multilingual-v0.1). A `Qwen3-0.6B` backbone autoregressively emits **kyutai/mimi** audio tokens (32 codebooks), which the Mimi codec decodes to 24 kHz audio. Supports **English and Japanese**. There is no separate upstream package — inference is the plain `transformers` snippet from the model card, so the wrapper runs it in-process (Python 3.12 venv with `torch==2.9.1+cu128` / `transformers==5.0.0`). Weights ([Vyvo](https://huggingface.co/Vyvo/Vyvo-Multilingual-v0.1) + [kyutai/mimi](https://huggingface.co/kyutai/mimi)) are ungated — no `HF_TOKEN` needed.

**Voices.** Vyvo is fundamentally a zero-shot cloning model with **no built-in speaker**, so every request needs a reference audio *and* its transcript. Set both `--vyvo-prompt-wav` and `--vyvo-prompt-text` and call with `voice="clone"` (`voice="default"` maps to the same path). Without them the wrapper returns a 4xx, mirroring the GPT-SoVITS contract — it does not silently fall back to a random voice. Sampling is exposed via `--vyvo-temperature` / `--vyvo-top-p` / `--vyvo-repetition-penalty` / `--vyvo-max-new-tokens` / `--vyvo-min-new-tokens` (defaults match the model card: 0.7 / 0.9 / 1.1 / 9600 / 960). For voice cloning, only use reference audio you have rights to (consent of the speaker).

> **Colab verification pending.** The code paths are in place, but end-to-end Colab verification (trycloudflare public URL + `/v1/audio/speech`) has not been run yet.

License: code and the Vyvo weights are both **Apache-2.0** (commercial use OK). However, inference always loads the **kyutai/mimi** codec, which is **CC-BY-4.0** — commercial use is permitted but **attribution to Kyutai is required**. See the license table below.
...
| Vyvo-Multilingual | Apache 2.0 | Apache 2.0 | OK | 0.9B Qwen3-0.6B-backbone LLM-TTS. EN / JA. Zero-shot cloning only (no built-in speaker; reference audio + transcript required). In-process via transformers; ungated download (no HF_TOKEN) |
