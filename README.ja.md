...
| Irodori-TTS | GPU 必須（デフォルトは v4.1-Small、v4-Small は L4 で確認済み） | 日本語 | OK |
...
v4.1-Small は、v4-Small の VoiceDesign caption、スタイル制御付き Voice cloning、合計最大 120 秒の参照音声への対応を引き継いでいます。これらの入力は、現時点では本ラッパーの `/v1/audio/speech` 契約には含めていません。FP32 チェックポイントは約 3GB のため GPU 利用を推奨します。従来の v4-Small デフォルト構成は NVIDIA L4 の Colab ランタイムで検証済みです。インストールと起動が完了し、OpenAI 互換 `/v1/audio/speech` がローカルおよび公開 `trycloudflare` 経由で 48kHz WAV を返すことを確認しました。caption-only の VoiceDesign も、同じランタイム上で上流ランタイムを直接使用して別途確認しています。新しい v4.1-Small デフォルト構成については、同じ Colab でのエンドツーエンド検証が必要です。

### Irodori-TTS-Anime

[`phasefield-audio/Irodori-TTS-v4.1-Anime`](https://huggingface.co/phasefield-audio/Irodori-TTS-v4.1-Anime) を使用する独立エンジンです。`Aratako/Irodori-TTS-v4.1-Small` をアニメ調の音声データで追加学習したモデルですが、配布元は第三者の `phasefield-audio` であり、Aratako公式リリースではありません。そのため、公式 `Irodori-TTS` のチェックポイント選択肢とは分離しています。推論には上流のIrodori-TTSランタイムと `Aratako/Semantic-DACVAE-Japanese-32dim` コーデックを再利用します。
...
Vyvo の 0.9B の LLM ベース TTS です。`Qwen3-0.6B` backbone が **kyutai/mimi** の音声トークン（32 codebook）を自己回帰生成し、Mimi codec が 24 kHz 音声へデコードします。独立した上流パッケージは無く、推論はモデルカードの `transformers` スニペットそのものなので、本リポジトリでは in-process で実行します（Python 3.12 venv に `torch==2.9.1+cu128` / `transformers==5.0.0` を導入）。いずれの checkpoint も ungated で `HF_TOKEN` 不要です。

**checkpoint**（`--vyvo-hf-model` で選択）:
