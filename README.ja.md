...
| Vyvo-Multilingual | **Colab 未検証** — コードは実装済みだが end-to-end の Colab 検証は未実施（GPU 推奨、VRAM 約2-4GB 想定。0.9B の Qwen3-0.6B backbone LLM-TTS で kyutai/mimi トークンを生成 → 24kHz。上流リポジトリ無し、transformers で in-process 実行。重みは ungated で `HF_TOKEN` 不要）。**ゼロショットクローン専用 — 内蔵スピーカー無し**: 各リクエストで `--vyvo-prompt-wav` + `--vyvo-prompt-text` が必須（未設定は 4xx）。コード / Vyvo 重みは Apache-2.0、kyutai/mimi コーデックは CC-BY-4.0（帰属表示要） | 英語 / 日本語 |
...
[Vyvo/Vyvo-Multilingual-v0.1](https://huggingface.co/Vyvo/Vyvo-Multilingual-v0.1) を使う 0.9B の LLM ベース多言語 TTS です。`Qwen3-0.6B` backbone が **kyutai/mimi** の音声トークン（32 codebook）を自己回帰生成し、Mimi codec が 24 kHz 音声へデコードします。**英語と日本語**に対応します。独立した上流パッケージは無く、推論はモデルカードの `transformers` スニペットそのものなので、本リポジトリでは in-process で実行します（Python 3.12 venv に `torch==2.9.1+cu128` / `transformers==5.0.0` を導入）。重み（[Vyvo](https://huggingface.co/Vyvo/Vyvo-Multilingual-v0.1) + [kyutai/mimi](https://huggingface.co/kyutai/mimi)）は ungated で `HF_TOKEN` 不要です。

**音声**: Vyvo は本質的に**内蔵スピーカーを持たない**ゼロショットクローンモデルで、各リクエストに参照音声 *と* その書き起こしの両方が必要です。`--vyvo-prompt-wav` と `--vyvo-prompt-text` の両方を設定し、`voice="clone"`（`voice="default"` も同じ経路）で呼び出してください。未設定の場合は GPT-SoVITS と同様に 4xx を返し、ランダム音声へ暗黙にフォールバックすることはありません。サンプリングは `--vyvo-temperature` / `--vyvo-top-p` / `--vyvo-repetition-penalty` / `--vyvo-max-new-tokens` / `--vyvo-min-new-tokens` で調整できます（既定値はモデルカード準拠: 0.7 / 0.9 / 1.1 / 9600 / 960）。クローンに使う参照音声は、権利のあるもの（話者の同意）のみを使用してください。

> **Colab 検証は未実施です。** コードは実装済みですが、end-to-end の Colab 検証（trycloudflare 公開 URL + `/v1/audio/speech`）はまだ実行していません。

ライセンス: コードと Vyvo の重みはいずれも **Apache-2.0**（商用利用可）です。ただし推論では常に **kyutai/mimi** コーデックを読み込みます。これは **CC-BY-4.0** で、商用利用は可能ですが **Kyutai への帰属表示が必要**です。下記のライセンス表も参照してください。
...
| Vyvo-Multilingual | Apache 2.0 | Apache 2.0 | OK | 0.9B の Qwen3-0.6B backbone LLM-TTS。EN / JA。ゼロショットクローン専用（内蔵スピーカー無し、参照音声 + 書き起こしが必須）。transformers で in-process 実行。ungated（HF_TOKEN 不要） |
