...
def resolve_selected_voice(settings: Settings) -> str:
    ...
    if settings.engine == "Vyvo-Multilingual":
...
def print_engine_voice_hints(settings: Settings):
    ...
    if settings.engine == "Fish-Speech":
        ...
        print(f"トークナイザ: {settings.misotts_tokenizer_repo}（上流ハードコードの gated meta-llama/Llama-3.2-1B を ungated ミラーへリダイレクト）。")
        print("          既定の unsloth/Llama-3.2-1B はバイト同一なので HF_TOKEN もライセンス同意も不要です。")
        print("          公式ソースを使う場合は --misotts-tokenizer-repo meta-llama/Llama-3.2-1B（HF_TOKEN + 同意が必要）。")
        ...
        print("          実行時に読む Llama 3.2 トークナイザ（ミラー / 公式いずれも）は Llama 3.2 Community License の対象です。")
        ...
        print("Vyvo-Multilingual は Vyvo の LLM ベース多言語 TTS です（Qwen3-0.6B backbone + kyutai/mimi codec、英語 / 日本語）。")
        ...
        print("voice 候補: clone（== default。いずれも参照音声が必須）")
        if settings.vyvo_prompt_wav and settings.vyvo_prompt_text:
            print(f"             参照音声: {settings.vyvo_prompt_wav} / 書き起こし: {settings.vyvo_prompt_text}")
        else:
        ...
        print("対応言語: 英語 / 日本語。出力は 24 kHz（mimi codec）。")
        ...
        print("ライセンス: コード / Vyvo 重みは Apache-2.0（商用 OK）。ただし kyutai/mimi コーデックは CC-BY-4.0 で帰属表示が必要です。")
