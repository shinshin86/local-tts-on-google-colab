...
def install(settings: Settings) -> dict:
    engine_dir = settings.engines_dir / "piper-openai"
    ...
    run([str(python_bin), "-m", "piper.download_voices", settings.piper_voice], cwd=str(engine_dir))
    backend_proc = popen(
        [
            str(python_bin),
            "-m",
        ...
        log_path=settings.log_dir / "piper-backend.log",
    )
    if not wait_http(f"http://127.0.0.1:{settings.piper_backend_port}/voices", timeout=120):
        tail_log(settings.log_dir / "piper-backend.log")
    ...
    env = {
        **os.environ,
        ...
        "OPENAI_MODEL_ID": settings.openai_model_id or settings.piper_voice,
        "PROXY_DEFAULT_VOICE": settings.piper_voice,
        "PROXY_SPEAKER_ID": str(settings.piper_speaker_id),
        "PROXY_ENGINE": "Piper",
    }
    proc = popen(
        [
        ...
        cwd=str(engine_dir),
        env=env,
        log_path=settings.log_dir / "piper-proxy-uvicorn.log",
    )
    return {
        "proc": proc,
        ...
        "app_dir": engine_dir,
        "log_path": settings.log_dir / "piper-proxy-uvicorn.log",
        "backend_log_path": settings.log_dir / "piper-backend.log",
    }
